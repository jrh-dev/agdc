...
import character_model, dbi, schemas
...
async def get_character(character_id: int, db: Session = Depends(get_db)):
    ...
    logging.info("Get character with ID %d endpoint accessed", character_id)
...
async def update_character(
    character_id: int, character: schemas.Character, db: Session = Depends(get_db)
):
    """Update a character by ID"""
    logging.info("Update character with ID %d endpoint accessed", character_id)
    db_character = dbi.DBI(db).update_character(
...
async def delete_character(character_id: int, db: Session = Depends(get_db)):
    ...
    logging.info("Delete character with ID %d endpoint accessed", character_id)
